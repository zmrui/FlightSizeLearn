...
def parse_time(log_path):
    with open(log_path, "r") as f:
        data = f.read()
        transtime = re.findall(r': (\d+\.\d+) seconds', data)
        return transtime[0]

def parse_receiver_goodput(log_path):
    try:
        transtime = parse_time(log_path)
        # print(goodputMbps)
        return float(transtime)
    except:
        return None
...
def plot_boxplot_mbps_list(boxplots):
    ...
    plt.savefig(f"wgwt_bbr_reorder{key}.png")
    plt.savefig(f"wgwt_bbr_reorder{key}.pdf")
...
if __name__ == "__main__":
    base = "/home/ubuntu/FlightSize/Results/wget"
    ...
    for exp in exp_path:
        if "MLON_bbr_reorder" in exp:
            boxplots = {}
            MLON_path = exp
            MLOFF_path = exp.replace("MLON_bbr_reorder", "MLOFF_bbr_reorder").replace("MLON_cubic_reorder", "MLOFF_cubic_reorder")
